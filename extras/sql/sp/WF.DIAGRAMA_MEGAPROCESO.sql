CREATE OR REPLACE PROCEDURE WF.DIAGRAMA_MEGAPROCESO(x_sesion BIGINT, x_proceso_inst_cl BIGINT)
  SPECIFIC WF.DIAGRAMA_MEGAPROCESO
DYNAMIC RESULT SETS 2
  ...
  (SELECT p.PROCESO,
    ...
    WHERE p.MEGA_PROCESO = (SELECT p1.MEGA_PROCESO
                             FROM wf_inst.PROCESOS p1
                             WHERE p1.CLAVE = x_proceso_inst_cl)
   WITH UR)
  ...
  DECLARE cursor2 CURSOR WITH RETURN FOR
    SELECT dc.clave,
           ...
           dc.CSS_CLASS
    FROM WF_OBJ.DIAGRAMA_CONECCION dc,
  ...
  SET x_megaproceso_obj_cl = (SELECT mp.MEGA_PROCESO
                                FROM wf_inst.MEGA_PROCESOS mp,
                                     wf_inst.PROCESOS p
                                WHERE p.MEGA_PROCESO = mp.clave AND
                                      p.CLAVE = x_proceso_inst_cl
